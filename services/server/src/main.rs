...
async fn init_apalis_pool(
    ...
    // don't put lock_timeout on pooled backends. statement_timeout is later,
    // not exempt: it covers the lock wait too, so it still cancels a blocked
    // statement, just at 300s instead of 15s.
