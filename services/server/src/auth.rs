...
/// Lightweight auth middleware for /sponsor and /sponsor/execute.
///
/// Verifies the Ed25519 signature mathematically (same as verify_signature)
/// but skips on-chain account resolution. This allows SetupWizard to get
/// gasless transactions before the delegate key is registered on-chain.
///
/// Security unchanged:
/// - Request must be signed with a valid Ed25519 key — cannot be forged
/// - Rate limiting still applies (keyed by public key)
/// - 5-minute timestamp window still enforced
/// - Unauthenticated curl still returns 401
pub async fn verify_signature_sponsor(
    ...
    let verifying_key = VerifyingKey::from_bytes(&pk_array).map_err(|_| StatusCode::UNAUTHORIZED)?;
    ...
    verifying_key.verify(message.as_bytes(), &signature).map_err(|e| {
        tracing::warn!("Sponsor signature verification failed: {}", e);
        StatusCode::UNAUTHORIZED
    })?;

    // No on-chain lookup — sponsor flow works before key is registered.
    // Use public_key as owner proxy so rate limit buckets are per-key,
    // not shared across all unregistered keys (which would collapse to "rate:").
    parts.extensions.insert(AuthInfo {
