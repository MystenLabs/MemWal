...
--
-- WALM-296: per-memory storage expiry. end_epoch is the on-chain Walrus
-- epoch the blob's storage lease ends at; expires_at is the computed
-- wall-clock equivalent (see services/server/src/sui/client.rs's new
-- staking-state epoch_duration/first_epoch_start read, Task 3 of this
-- plan). expiry_synced_at is bookkeeping for the lazy/periodic refresh
-- sweep (Task 8) — NOT touched by anything else, and this migration does
...
-- WALM-295's updated_at migration, no backfill/NOT NULL step is needed
