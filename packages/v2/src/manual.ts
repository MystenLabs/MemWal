 ...
 * @example
 * ```typescript
 * import { MemWalManual } from "@cmdoss/memwal-v2"
 *
...
export class MemWalManual {
    ...
    /**
     * Remember (manual/full client-side):
     * 1. Embed text (OpenAI)
     * 2. SEAL encrypt (user's wallet)
     * 3. Upload to Walrus (user signs)
     * 4. Register {blobId, vector} with server
     */
    async rememberManual(text: string): Promise<RememberManualResult> {
        ...
        // Step 4: Register vector + blobId with server
