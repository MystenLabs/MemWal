...
const context = [
    ...
    "You decide from meaning, in any language or spelling. Save with memwal_remember only when a fact will still matter later (stable preference, project decision, hard constraint, correction, identity) — writes are expensive, so skip one-off tasks, the current file or bug, and small talk. Before tasks that reference past work or preferences, recall with memwal_recall.",
