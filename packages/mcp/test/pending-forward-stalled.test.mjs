...
/** A relayer that serves exactly one healthy session and then refuses every
 * reconnect. This is the mid-session shape, and it is the one Dio and Teo
 * actually reported: the bridge worked, then the relayer stopped answering.
 * `startUnavailableRelayer` above cannot reach it — it fails from the very
 * first handshake, so the request lands in `pendingForward`, which is only
 * ever filled before the first successful connect. */
function startHealthyThenDeadRelayer() {
    ...
    const server = http.createServer((req, res) => {
        ...
        if (req.method === "GET" && url.pathname === "/api/mcp/sse") {
            ...
            sseGetCount += 1;
            if (sseGetCount > 1) {
                // Every reconnect after the first session is refused, the way
                // the relayer does while the delegate verify cannot reach a
                // throttled fullnode.
                res.writeHead(503, { "content-type": "text/plain" });
        ...
        if (req.method === "POST" && url.pathname === "/api/mcp/messages") {
            res.writeHead(hasBridgeAuth(req) ? 202 : 401);
            res.end();
            return;
        }
        res.writeHead(404);
...
test("a call issued after the session dies is answered on the stalled deadline", async (t) => {
    // The gap this closes: `neverSent` used to be read from `pendingForward`
    // membership, and nothing refills that buffer once `firstConnectDone` is
    // set. So in the mid-session outage — the reported one — the call looked
    // "sent", kept the full call timeout, and came back blaming a dropped
    // connection for a request that never left the process.
    ...
    // Wait for the first session to actually come up — `firstConnectDone` has
    // to be set, or this degenerates into the cold-start case already covered.
    const upBy = Date.now() + 10_000;
    while (!mock.sessionIsUp() && Date.now() < upBy) {
        await new Promise((r) => setTimeout(r, 50));
    }
    assert.ok(mock.sessionIsUp(), "the first session must connect, or this tests the wrong path");
    ...
    const reply = await waitFor((m) => m.id === 2, 30_000);
    const waitedMs = Date.now() - sentAt;
    assert.ok(
        waitedMs < CALL_TIMEOUT_MS,
        `a call that never left the process must expire on the ${STALLED_HANDSHAKE_MS}ms stalled ` +
            `deadline, not the ${CALL_TIMEOUT_MS}ms call timeout — waiting out the latter with no ` +
            `feedback is the reported bug; waited ${waitedMs}ms`,
    );
    assert.equal(
        reply.result?.isError,
        true,
    ...
    assert.match(
        text,
        /could not reach the relayer/i,
        `the answer must name the failing connection, not blame a dropped reply, got ${text}`,
    );
    assert.match(
        text,
        /nothing was\\?\s*stored/i,
