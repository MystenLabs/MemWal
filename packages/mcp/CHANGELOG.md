...
- Approval covers the account and relayer, not just the directory. Adopting a directory alone made approval a one-time gate on a file whose contents keep changing — the file lives in a repository, so a `git pull`, a merged PR or a rebase can rewrite `accountId` or `relayerUrl` afterwards and a directory adopted last month would carry the new values without asking, which is the same defect with one extra step. The ledger records the pair the user saw, and a mismatch stops the file being used until it is approved again, naming both the approved and the current pair so an expected change is distinguishable from one the user did not make. A rotated delegate key or a relabel under the same account and relayer is not a change.
