...
- Raise the remember bar: save only facts that will still matter in a later session or on another machine. Writes are expensive, so skip one-off tasks, the current file or bug, small talk, and duplicates.
