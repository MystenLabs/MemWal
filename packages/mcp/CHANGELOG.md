...
- Pin the launcher configs to `@mysten-incubation/memwal-mcp@latest`. Unpinned, `npx` recorded `^0.0.5` in its cache, and a caret on a `0.0.x` version locks the exact patch — so a machine kept launching 0.0.5 for weeks after 0.0.9 through 0.0.12 shipped, and updating the plugin did not move it.
