...
- Answer orphaned tool calls whose upstream response never arrives with a retryable error instead of hanging indefinitely. The bridge now tracks in-flight request start times and sweeps expired calls via a per-request deadline (`MEMWAL_MCP_CALL_TIMEOUT_MS`, default 240s), drops late responses to avoid duplicate replies, and enriches reconnect logs with pending request IDs and methods. (WALM-328, #690)
