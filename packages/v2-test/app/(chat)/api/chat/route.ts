...
export async function POST(request: Request) {
  ...
  try {
    const { id, message, messages, selectedChatModel, selectedVisibilityType, useMemWal } =
      requestBody;
    ...
    const stream = createUIMessageStream({
      ...
      execute: async ({ writer: dataStream }) => {
        const result = streamText({
          model: useMemWal !== false ? getMemWalModel(selectedChatModel) : getLanguageModel(selectedChatModel),
