 ...
 *     owner: address,
 *     version: u64,
 *     index_blob_id: String, // Pointer to index.hnsw on Walrus
 *     graph_blob_id: String  // Pointer to graph.json on Walrus
 * }
 * ```
 */
export interface MemoryIndex {
  ...
  /** Owner address */
  owner: string;
  /** Version for optimistic locking (increments on each update) */
  version: number;
  /** Blob ID pointing to serialized HNSW index on Walrus */
  indexBlobId: string;
  /** Blob ID pointing to knowledge graph JSON on Walrus */
  graphBlobId: string;
}
