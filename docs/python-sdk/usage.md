...
Bulk (up to 20 items per call) follows the same pattern with `remember_bulk_async`, `wait_for_remember_jobs`, and `remember_bulk_and_wait`. Polling starts immediately, then grows 1.5× toward a 5s cap with ±25% jitter. Backoff is disabled above the cap.
