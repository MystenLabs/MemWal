...
This release adds `failed` on `restore()` results for permanent decrypt and UTF-8 failures, and grows remember-job poll delay 1.5× toward a 3s cap with an immediate first poll.
