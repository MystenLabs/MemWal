...
answer: >-
  The latest Python SDK release is 0.1.9. It adds `failed` on `restore()` results for permanent decrypt/UTF-8 failures, and rejects empty `remember_bulk_async` batches and misaligned relayer `job_ids`. 0.1.8 added `dropped_count` on recall results, `write_ready` on health, `MemWalClockDriftError` for clock-drift 401s, and the `dev` relayer preset.
---
...
This release reports restore decrypt failures instead of folding them into `skipped`, and aligns Python bulk remember with the TypeScript SDK by refusing empty batches and mismatched job ids.
