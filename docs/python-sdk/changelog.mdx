...
answer: >-
  The latest Python SDK release is 0.1.9. It warns when `server_url` uses plaintext HTTP on a non-localhost host. 0.1.8 adds `dropped_count` on recall results, `write_ready` on health, `MemWalClockDriftError` for clock-drift 401s, and the `dev` relayer preset, and hardens hex decoding and error redaction.
---
...
This release warns when `server_url` uses plaintext HTTP on a non-localhost host.
