...
(Final formula, post-implementation fix round — anchored on the CURRENT epoch and wall-clock `now`, not genesis-anchored on `first_epoch_start`. The genesis-anchored formula this spec originally proposed extrapolates from a single fixed point over potentially hundreds of epochs, with no self-correction if `epoch_duration` ever changes or actual epoch rollovers drift from the ideal schedule. Anchoring on the current epoch instead self-corrects every sweep cycle (the 24h resync re-fetches both `current_epoch` and `now`), bounding the error to at most one epoch's drift since the last resync. `first_epoch_start` is still read off the staking pool object by `parse_epoch_schedule`, but is no longer used by the conversion itself.)
...
- ~~Confirm whether Walrus `epoch_duration` has ever changed or can change. If it can, the linear `expires_at` formula silently mis-computes for any lease spanning a change boundary, and the 24h lazy refresh does not self-correct (it reapplies the same formula).~~ **Resolved** by the current-epoch-anchoring switch above: the formula no longer extrapolates from a fixed genesis point, so it self-corrects every sweep cycle regardless of whether `epoch_duration` changes — a duration change (or schedule drift) can no longer silently corrupt long-extrapolated dates. Whether `epoch_duration` can change is no longer load-bearing for correctness.
