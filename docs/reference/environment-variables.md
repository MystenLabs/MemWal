...
| `MEMWAL_CREDS_DIR` | none | `~/.memwal` | Directory holding `credentials.json`. Resolved on every access, so it can be pointed elsewhere without restarting. Mainly for tests, which must not write into the real credential directory |
