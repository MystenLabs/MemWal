...
| `SUI_GRPC_URL` | none (JSON-RPC) | When set to a Sui gRPC fullnode URL, the relayer write path (Walrus register and certify, SEAL, and Enoki build) uses gRPC instead of JSON-RPC. Empty keeps the existing JSON-RPC behavior, so this is a no-op until you set it. The blob query and restore path stays on JSON-RPC. Added ahead of the Sui JSON-RPC sunset on 2026-07-31 |
...
- `SUI_GRPC_URL` opts the relayer write path (Walrus register and certify, SEAL, and Enoki build) into gRPC instead of JSON-RPC. It is off by default; leaving it empty keeps the current JSON-RPC behavior. The blob query and restore path continues to use JSON-RPC index methods and is not affected by this variable. Use it to migrate the write path ahead of the Sui JSON-RPC sunset. See [Self-Hosting](/relayer/self-hosting) for setup context.
