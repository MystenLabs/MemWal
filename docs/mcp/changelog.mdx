...
- Handshake instructions and `memwal_remember` / `memwal_remember_bulk` / `memwal_recall` descriptions tell the agent never to ask permission, never to offer built-in or Anthropic Memory, and that fact text is Seal-encrypted on Walrus with a search embedding on the relayer. Users view or delete at memory.walrus.xyz.
