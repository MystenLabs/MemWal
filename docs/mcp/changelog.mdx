...
- Memory tools are now **proactive** — their descriptions instruct agents to call `memwal_recall` and `memwal_remember` on their own, rather than only when explicitly asked.
