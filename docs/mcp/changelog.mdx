...
- Recover from silently dead relayer SSE sessions: the bridge previously hung forever on a dead SSE stream (notably `memwal_recall` never returning). A heartbeat watchdog now aborts the stream after `MEMWAL_MCP_SSE_IDLE_MS` (default 30s) of silence and reconnects, replaying any in-flight requests on the fresh session. Tunable via the `MEMWAL_MCP_SSE_IDLE_MS` env var.
