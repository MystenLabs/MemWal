...
- The SEAL `SessionKey` is refreshed proactively. Once a cached session enters the last minute of its usable life, a request still gets the cached credential immediately and the rebuild happens out of band instead of inline. Long-running batches no longer stall roughly every 4m30s. The 5-minute TTL is unchanged.
- SEAL session failures carry a legible message and a `status` the documented `withRetry` helper can classify: `503` for transient build failures, `400` for a missing or too-old `@mysten/sui` / `@mysten/seal`, and `400` when a rebuilt session is still rejected. Previously these threw before any `fetch` and reached callers with no `status` at all.
