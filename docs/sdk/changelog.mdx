...
answer: >-
  The latest TypeScript SDK release is 0.1.6. It adds optional `failed` on `restore()` results for permanent decrypt/UTF-8 failures. 0.1.5 added `dropped_count` on recall results and `write_ready` on health, switched `rememberManual` to sending `encryptedData`, and hardened hex decoding and error redaction.
---

## 0.1.6

This release reports restore decrypt failures instead of folding them into `skipped`.
